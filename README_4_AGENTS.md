...
|----------|-------|
| Binary | `projectsCLI` |
| Config file | `~/.openclaw/config.toml` (TOML) |
| Data directory | `~/.openclaw/projects/` |
...
| `--config` | string | `~/.openclaw/config.toml` | Config file path |
...
- Writes template files: `memory/MEMORY.md`, `context/CONTEXT.md`, `tasks/TODO.md`, `docs/README.md`, `.gitignore`
...
### Config Schema (`~/.openclaw/config.toml`)

```toml
projects_dir = "~/.openclaw/projects"   # Path to projects directory
...
|-------|------|---------|-------------|
| `projects_dir` | string | `~/.openclaw/projects` | Where projects are stored |
...
`~/.openclaw/projects/PROJECTS.md` — Auto-generated markdown table of all projects. Regenerated on `create` and `delete`.
...
cat ~/.openclaw/projects/my-project/PROJECT.md

# Memory file for persistent context
cat ~/.openclaw/projects/my-project/memory/MEMORY.md
```
